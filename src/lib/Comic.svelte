<script lang="ts">
	...
	async function getId(): Promise<number> {
		...
		const res: Response = await fetch('https://fwd.innopolis.app/api/hw2?' + params.toString());
	...
	async function getComic(id: number): Promise<Comic> {
		...
		const res: Response = await fetch('https://getxkcd.vercel.app/api/comic?' + params.toString());
